...
arr=(`oci iam compartment list --query 'data[][].[name, id]' --output json | jq -r '.[] | @tsv'`)
...
echo "Select compartment to use:"
echo

select_option "${k[@]}"
choice=$? 

oci_compartment_ocid=${v[$choice]}
...
fn update context registry ${region}.ocir.io/${ns}/${oci_repo_name}
